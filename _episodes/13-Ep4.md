...
Once on the group landing page, select the resource titled "Introduction to TauDem and click "Open with" at the top right. Choose CUAHSI JupyterHub as the webapp. Agree to the terms of use and click "Sign in with HydroShare". Authorize CUAHSI JupyterHub. The JupyterHub seesion will open. Select the file at the top of the list named, TauDEM.ipynb.


### Step 2

Note that Professor Tarboton's comments are included in the notebook to help users find documenation and understand what task each cell performs.

The first cell in the notebook is like most Jupyter Notebooks. All necessary software are installed in the users JupyterHub using the pip command. Module imports are also carried out here. The hstools module provides functions for interacting with HydroShare, including resource querying, dowloading and creation. The taudem module provides functions for workspace maintenance, data analysis as well as visualization. 
...
Calling the previously defined plotting function, resulting files can be visualized.
...
## Save the Results back into HydroShare (*THIS IS A STEP)

*The hstools library provides functions for JupyterHub to interact with HydroShare. For more information on how to us hstools, users can run the following commands.
...
*To create a new HydroShare resource containing the notebook which we made additions to, we can use the "!hs create" command in a separate notebook cell as follows.
...
Users can view all files which will be included in the new respurce. 
...
*Metadata can be defined within JupyterHub using Python programming language as shown below. Notice that fields required for resource creation include title, abstract, keywords, and content files.
...
Here we will go through the Jupyter notebook Dr. Tarboton has provided us with the intention to construct other analyses to meet our research needs as we become familiar with the computational tasks TauDEM helps us solve.
...
Recall that 'dem' is out logan.tif file.
